...
Quando usando `TRANSPORT=sse`, o servidor MCP usa o protocolo Server-Sent Events (SSE) que requer um fluxo específico de comunicação:

### Fluxo de Comunicação SSE

**IMPORTANTE para LLMs**: O cliente deve seguir este fluxo na ordem correta:

1. **Estabelecer Conexão SSE** (GET /sse):
   ```bash
   curl -N http://localhost:8152/sse
   ```
   O servidor retorna um evento `endpoint` com o `session_id`:
   ```
   event: endpoint
   data: /messages/?session_id=abc123def456...
   ```

2. **Enviar Mensagens JSON-RPC** (POST /messages/?session_id=XXX):
   ```bash
   curl -X POST "http://localhost:8152/messages/?session_id=abc123def456" \
...
|----------|--------|-----------|
| `/sse` | GET | Estabelece conexão SSE e retorna `session_id` |
| `/messages/?session_id=XXX` | POST | Recebe mensagens JSON-RPC do cliente |
...
# 1. Conectar ao SSE e obter session_id
...
for event in client.events():
    if event.event == 'endpoint':
        # Extrai session_id do endpoint: /messages/?session_id=XXX
        session_id = event.data.split('session_id=')[1]
        break

# 2. Usar session_id para enviar mensagens
...
O servidor também fornece endpoints REST para gerenciamento de sessões HTTP (independente das sessões SSE):

| Endpoint | Método | Descrição |
|----------|--------|-----------|
| `/sessions` | POST | Cria nova sessão HTTP com metadados |
| `/sessions/{session_id}` | GET | Obtém informações da sessão |
| `/sessions/{session_id}` | DELETE | Deleta sessão |
