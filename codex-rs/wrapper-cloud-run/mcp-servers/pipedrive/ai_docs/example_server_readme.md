...
1. Build the Docker image:
   ```bash
   docker build -t mcp/mem0 --build-arg PORT=8151 .
   ```
...
| `PORT` | Port to listen on when using SSE transport | `8151` |
...
```bash
docker run --env-file .env -p:8151:8151 mcp/mem0
```
...
>     "mem0": {
>       "transport": "sse",
>       "serverUrl": "http://localhost:8151/sse"
>     }
>   }
> }
> ```
...
> 
> So the full URL in the MCP node would be: http://host.docker.internal:8151/sse

Make sure to update the port if you are using a value other than the default 8151.
